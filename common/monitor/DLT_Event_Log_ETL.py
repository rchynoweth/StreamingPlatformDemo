...
event_log_path = "/demos/dlt/loans/ryan_chynoweth/"
dirs = [f.path for f in dbutils.fs.ls("/databricks-datasets")]
assert len(dirs) > 0
...
def raw_event_log():
  df = (spark.readStream
        .format('delta')
        .load(dirs[0]+"system/events")
        .withColumn("bronze_load_datetime", current_timestamp())
       )
  
  for i in range(1,len(dirs)):
    df = df.union(spark.readStream
        .format('delta')
        .load(dirs[i]+"system/events")
        .withColumn("bronze_load_datetime", current_timestamp())
       )
